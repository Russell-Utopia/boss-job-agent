...
`platform_jobs` 的所有写入统一经过进程内 `JobPool` 模块；岗位发现、岗位鉴定、首次沟通和 Web 都只能提交“观察到岗位”“记录人工结论”“完成鉴定”“完成沟通”等业务意图，不能直接设置状态或逐字段更新。这样 JD 变化触发重新鉴定并撤回待发送、人工结论变化影响发送资格、发送前发现岗位关闭等跨流程规则，可以在一个 SQLite 事务中校验并落库。
